...
class ProductViewModel(application: Application): AndroidViewModel(application) {
    ...
    fun fetchNextPage(): LiveData<List<DocumentSnapshot>> {
        ...
        getFirestore().collection("Product")
            .orderBy("someField")
     ...
     **/
    fun allProduct() {
        getFirestore().collection("Product").addSnapshotListener { products, e ->
    ...
    fun viewCountUp(pId: String, pName: String) {
        val dbRef = getFirestore().collection("Product")
    ...
    fun bidchange(pId: String, pName: String, bidPrice: String) {
        ...
        dbRef.whereEqualTo("ID", pId).whereEqualTo("productName", pName).get()
            .addOnSuccessListener { productSnapshot ->
                productSnapshot.documents.forEach { document ->
                    val update = mapOf("productBidPrice" to bidPrice)
                    dbRef.document(document.id).set(update, SetOptions.merge())
                }
            }
    }
    ...
    fun updateProduct(pId: String, pName: String) {
        getFirestore().collection("Product").whereEqualTo("ID", pId).whereEqualTo("productName", pName).get()
            .addOnSuccessListener { result ->
                val currentTime = Timestamp.now()
                result.forEach { document ->
                    document.reference.update("insertTime", currentTime)
                        .addOnSuccessListener {
            ...
            .addOnFailureListener { e ->
                Log.e("Firestore", "Error getting product for update", e)
            }
    }
